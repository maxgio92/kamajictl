...
func (c *TCPClient) ListTCPs(ctx context.Context, opts *TCPOptions) (*kamajiv1alpha1.TenantControlPlaneList, error) {
	...
	if err := c.kube.List(ctx, tcpList, &client.ListOptions{
		Namespace: opts.Namespace,
	}); err != nil {
		return nil, err
	}
...
func (c *TCPClient) GetKubeconfig(ctx context.Context, opts *TCPOptions) (string, error) {
	if err := c.Validate(); err != nil {
		return "", errors.Wrap(err, "error validating client options")
	}
	...
	if t.Status.KubeConfig.Admin.SecretName == "" {
		return "", fmt.Errorf("kubeconfig secret for tenantcontrolplane %s/%s not ready", opts.Name, opts.Namespace)
	}
	...
	return string(secret.Data["admin.conf"]), nil
}
